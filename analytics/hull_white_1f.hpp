...
class hull_white_1f {
   public:
    ...
    // surface
    hull_white_1f(const std::shared_ptr<const discount_curve>& curve, double a,
                  double sigma);
    ...
    static std::shared_ptr<hull_white_1f> calibrate(
        ...
        const std::shared_ptr<simple_vol_surface>& vol_surf, double a = 0.01,
        double tol = 1e-8);
    ...
    double price_cap_monte_carlo(date_t start_date, date_t end_date, double strike,
   ...
   private:
    ...
    struct short_rate_path_info {
        date_t valuation_date, start_date, end_date;
        double r_at_start;    // r(S)
        double int_0_to_end;  // ∫_0^E r dt
        const hull_white_1f& model;
        ...
        double discount_0_to_end() const { return std::exp(-int_0_to_end); }
    };
    ...
    // - dt: time step in years
    short_rate_path_info simulate_short_rate_to(date_t start_date, date_t end_date,
                                                double r0, double dt) const;
};
